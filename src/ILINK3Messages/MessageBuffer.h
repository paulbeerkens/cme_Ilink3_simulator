...
public:
    ...
    inline size_t getWrtBufFree () const { return bufSize_ - uintptr_t (wrtPtr_ - buf_); }
    inline size_t getLeftToRead () const { return uintptr_t (wrtPtr_ - rdPtr_); }

    bool readFromSocket (std::int32_t socket, std::size_t size);
    bool sendToSocket (std::int32_t socket);

    [[nodiscard]] const char* getRdPtr () const {return rdPtr_;}
    [[nodiscard]] char* getWrtPtr () const {return wrtPtr_;}
    void moveRdPtr (std::size_t bytes);
    void moveWrtPtr (std::size_t bytes);
protected:
    char* buf_    {nullptr};
    char* rdPtr_  {nullptr};
    char* wrtPtr_ {nullptr};
