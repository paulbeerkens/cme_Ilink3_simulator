...
int main (int argc, [[maybe_unused]]char** argv) {
    ...
    */
    ...
    std::ofstream enumFile_h;
    enumFile_h.open ("Generated/ILink3EnumsGen.h");

    enumFile_h << "//File generated by CodeGenerator" << std::endl;
    enumFile_h << "//Generated from: " << argv [1] << std::endl;
    enumFile_h << std::endl;

    enumFile_h << "#ifndef CMESIMULATOR_ILINK3_ENUMS_GEN_H" << std::endl;
    enumFile_h << "#define CMESIMULATOR_ILINK3_ENUMS_GEN_H" << std::endl;

    enumFile_h << "#include <cstdint>" << std::endl;
    enumFile_h << "#include <ostream>" << std::endl;
    enumFile_h << std::endl;

    enumFile_h << "namespace IL3Enum {" << std::endl;
    enumFile_h << std::endl;

    for (const auto& anEnum: enumDefinitions) {
        //enum class FTI: std::uint8_t {Backup=0, Primary=1, Null=255};
        enumFile_h << "enum class " << anEnum.second.name_;
        enumFile_h << ": " << anEnum.second.encodingType_->primitiveType_;
        enumFile_h << "{" << std::endl;
        std::string preFix {"  "};
        for (const auto&validValue: anEnum.second.validValues_) {
            enumFile_h << preFix << validValue.name_ << "=";
            if (anEnum.second.encodingType_->primitiveType_==PrimitiveType::char_t) {
                enumFile_h << "'" << validValue.value_ << "'";
            } else {
                enumFile_h << validValue.value_;
            }
            enumFile_h << " //" << validValue.description_ << std::endl;
            preFix=", ";
        }
        if (anEnum.second.encodingType_->nullValue.has_value()) {
            enumFile_h << preFix << "Null=" << *anEnum.second.encodingType_->nullValue;
        }
        enumFile_h << "};" << std::endl << std::endl;
    }
    enumFile_h << "} //end of namespace IL3Enum" << std::endl;
    enumFile_h<<std::endl;

    //Create streamout operator declerations
    //std::ostream& operator <<(std::ostream& os, FTI e)
    for (const auto& anEnum: enumDefinitions) {
        enumFile_h<<"std::ostream& operator <<(std::ostream& os, IL3Enum::"<<anEnum.second.name_<<" e);"<<std::endl;
    }

    enumFile_h << "#endif //CMESIMULATOR_ILINK3_ENUMS_GEN_H" << std::endl;
    enumFile_h.close ();

    std::ofstream enumFile_cpp;
    enumFile_cpp.open ("Generated/ILink3EnumsGen.cpp");

    enumFile_cpp << "//File generated by CodeGenerator" << std::endl;
    enumFile_cpp << "//Generated from: " << argv [1] << std::endl;
    enumFile_cpp << std::endl;

    enumFile_cpp << R"(#include "ILink3EnumsGen.h")"<<std::endl;
    enumFile_cpp << std::endl;

    //Create streamout operator definitions
    for (const auto& anEnum: enumDefinitions) {
        enumFile_cpp<<"std::ostream& operator <<(std::ostream& os, IL3Enum::"<<anEnum.second.name_<<" e) {"<<std::endl;
        if (anEnum.second.encodingType_->primitiveType_==PrimitiveType::char_t) {
            enumFile_cpp<<"\tos<<static_cast<char>(e);"<<std::endl;
        } else if (primitiveTypeIsUnsigned (anEnum.second.encodingType_->primitiveType_)){
            enumFile_cpp<<"\tos<<static_cast<std::uint64_t>(e);"<<std::endl;
        } else {
            enumFile_cpp<<"\tos<<static_cast<std::int64_t>(e);"<<std::endl;
        }
        enumFile_cpp<<"\treturn os;"<<std::endl;
        enumFile_cpp<<"}"<<std::endl;
    }
